    ...
    <>
          ...
          return (
            <div key={s.k} className="sym-card">
              <div className="sym-row">
                ...
                <button className="sym-info-btn" onClick={() => setOpenInfo(isOpen ? null : s.k)} aria-label={`Info om ${s.l}`}>ⓘ</button>
      ...
      {[
        ...
        <section key={id} className="section">
          ...
          <textarea ref={el => textRefs.current[id] = el} className="text-field" placeholder={ph} rows={2}
            value={data[id] || ''} onChange={e => onField(id, e.target.value)} />
        </section>
      ))}
...
export default function DayView({ user, gamification, onAwardXP, onSpin, spunToday, loggedToday, today }) {
  ...
  const load = useCallback(async () => {
    const { data: row } = await supabase
      ...
      .eq('user_id', user.id).eq('entry_date', dateKey).maybeSingle()
    setData(row || {})
  }, [user.id, dateKey])
  ...
  async function save() {
    ...
    if (isToday && !loggedToday) {
      ...
      let xp = XP_REWARDS.log_day + (habitCount * XP_REWARDS.habit_each) + (allHabits ? XP_REWARDS.all_habits_bonus : 0)
  ...
  return (
    <div className="day-wrap">
      {isToday && gamification && (
        <>
          <StreakBanner streak={gamification.current_streak || 0} loggedToday={loggedToday} />
          <XPBar totalXP={gamification.total_xp || 0} />
          <DailySpin today={today} spunToday={spunToday} onSpin={onSpin} />
        </>
      )}
      ...
      {dayIdx === 6 ? (
        ...
        <DayForm data={data} dayIdx={dayIdx} saving={saving} saved={saved}
